...
from loss import cal_loss,cal_loss_loupe
...
parser.add_argument('--loss_epsilon','-loss_epsilon', type=float, default=1e-2, help='ratio of ssim_loss in loss')
...
# def forward(mode, rank, model, dataloader, criterion, optimizer, log, args):
    ...
    if mode =='val':
        writer.add_image(f'images/val_under_realmask:{mask_show_under[:,0,:,:].sum()/(256*256)}'\
                         , mask_show_under[:,0,:,:].squeeze().unsqueeze(0), epoch) # 原始模拟降采mask实部
        writer.add_image(f'images/val_under_imgmask:{mask_show_under[:,1,:,:].sum()/(256*256)}'\
                         , mask_show_under[:,1,:,:].squeeze().unsqueeze(0), epoch) # 原始模拟降采mask虚部
        writer.add_image(f'images/val_loss_realmask:{mask_show_loss[:,0,:,:].sum()/(mask_show_under[:,0,:,:].sum())}'\
                         , mask_show_loss[:,0,:,:].squeeze().unsqueeze(0), epoch) # loss划分mask实部
        writer.add_image(f'images/val_loss_imgmask:{mask_show_loss[:,1,:,:].sum()/(mask_show_under[:,1,:,:].sum())}'\
                         , mask_show_loss[:,1,:,:].squeeze().unsqueeze(0), epoch) # loss划分mask虚部
        writer.add_image(f'images/val_dc_realmask:{mask_show_dc[:,0,:,:].sum()/(mask_show_under[:,0,:,:].sum())}'\
                         , mask_show_dc[:,0,:,:].squeeze().unsqueeze(0), epoch) # dc划分mask实部
        writer.add_image(f'images/val_dc_imgmask:{mask_show_dc[:,1,:,:].sum()/(mask_show_under[:,1,:,:].sum())}'\
                         , mask_show_dc[:,1,:,:].squeeze().unsqueeze(0), epoch) # dc划分mask虚部
        ...
        writer.add_image(f'images/train_under_realmask:{mask_show_under[:,0,:,:].sum()/(256*256)}'\
                         , mask_show_under[:,0,:,:].squeeze().unsqueeze(0), epoch) # 原始模拟降采mask实部
        writer.add_image(f'images/train_under_imgmask:{mask_show_under[:,1,:,:].sum()/(256*256)}'\
                         , mask_show_under[:,1,:,:].squeeze().unsqueeze(0), epoch) # 原始模拟降采mask虚部
        writer.add_image(f'images/train_loss_realmask:{mask_show_loss[:,0,:,:].sum()/(mask_show_under[:,0,:,:].sum())}'\
                         , mask_show_loss[:,0,:,:].squeeze().unsqueeze(0), epoch) # loss划分mask实部
        writer.add_image(f'images/train_loss_imgmask:{mask_show_loss[:,1,:,:].sum()/(mask_show_under[:,1,:,:].sum())}'\
                         , mask_show_loss[:,1,:,:].squeeze().unsqueeze(0), epoch) # loss划分mask虚部
        writer.add_image(f'images/train_dc_realmask:{mask_show_dc[:,0,:,:].sum()/(mask_show_under[:,0,:,:].sum())}'\
                         , mask_show_dc[:,0,:,:].squeeze().unsqueeze(0), epoch) # dc划分mask实部
        writer.add_image(f'images/train_dc_imgmask:{mask_show_dc[:,1,:,:].sum()/(mask_show_under[:,1,:,:].sum())}'\
                         , mask_show_dc[:,1,:,:].squeeze().unsqueeze(0), epoch) # dc划分mask虚部
